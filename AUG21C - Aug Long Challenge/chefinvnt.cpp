...
int main() {
	...
	cin>>t;
	while(t--){
	    long long int n, p, k, repeat, index, count=0, fcount;
	    cin>>n>>p>>k;
	    if(n%k == 0){
	        repeat = n/k;
	    }
	    ...
	    count = (p-index)/k;
	    count++;
	    fcount = (repeat*index) + count;
	   // cout<<"final count1: "<<fcount<<endl;
	   // cout<<"n mod k : "<<n%k<<endl;
	   // cout<<"p mod k : "<<p%k<<endl;
	    if(n%k != 0){
    	    if(n%k<=p%k){
    	        fcount = fcount - ((p%k) - (n%k));
    	        //cout<<"final count2: "<<fcount<<endl;
            }
	    }
